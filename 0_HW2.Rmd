...
subtitle: 'INFO 523'
author:
- name: Student -  Cristina Lafuente
...
```{r}
num_list <- c(34, 56, NA, 87, NA, 4, 77, NA, 21, NA, 39)
```


```{r}

q3 <- sum(is.na(num_list))
q3
```
...
q5 <- (c(FALSE, TRUE)) | (c(TRUE, TRUE)) 
q5 <- TRUE
```
Using the or operator, True or False is true and True or True is true. So the answer is a vector of length 2 with both values (here, true and true) then evaluated with the logical or for a final value of TRUE.
...
q9.function <- function(input){
  inches <- as.numeric(str_extract(input, "\\d*"))
  return(inches * 2.54)
}

q9 <- q9.function("3201 in")
q9
...
q10 <- list("totalInd"=length(vd), 
            "totalVd"=sum(unlist(vd)), 
            "less4Vd"=sum(vd<=4))
q10
```
...
```{r}
df11 <- data.frame(Age, Name, Gender)



q11.1 <- df11 |> 
  mutate(
  gender01 = case_when(Gender == "M" ~ 0,
                       TRUE ~ 1)
  ) 
q11.2 <- df11 |>
  mutate(
    gender01 = if_else(Gender == "M", 0, 1)
  )
q11.1
