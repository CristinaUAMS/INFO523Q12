...
Use R to count the number of NAs in the following vector:

```{r}
c(34, 56, 55, 87, NA, 4, 77, NA, 21, NA, 39)
```
...
What will be the output for the R expression `(c(FALSE, TRUE)) || (c(TRUE, TRUE))`? Briefly explain the logic of your result.
...
Write an R expression that will return the positions of 3 and 7 in the vector `x <- c(2, 3, 8, 2, 74, 1, 9, 9, 3, 2, 1)`
...
```{r}
# BEGIN SOLUTION
q12.2 <- NULL ##paste your link here (as a string)
# END SOLUTION
```

3. Create a new organization associated with your account (e.g. `firstname_DM523`). Below (`q12.3`), list the link to the organization.

```{r}
# BEGIN SOLUTION
q12.3 <- NULL ##paste your link here (as a string)
# END SOLUTION
```
...
```{r}
# BEGIN SOLUTION
q12.4 <- NULL ##paste your link here (as a string)
# END SOLUTION
```
...
```{r}
# BEGIN SOLUTION
q12.5 <- NULL ##paste your link here (as a string)
# END SOLUTION
```
...
```{r}
# BEGIN SOLUTION
q12.6 <- NULL ##paste your link here (as a string)
# END SOLUTION
```
